...
Function RemoveCommonPbFromStandard207(StdIntName)
	...
	For (i = 1; i<=NoOfIntegrations; i = i + 1)
		...
		Variable sAge = GetValueFromStandard("Age", ReferenceStandard)
		...
		If (numtype(c76) == 2)
			...
			Variable c74 = -0.034*(sAge/1e3)^4 +0.181*(sAge/1e3)^3 - 0.448*(sAge/1e3)^2 + 0.334*(sAge/1e3) + 15.64	
			Variable c64 = 0.023*(sAge/1e3)^3 - 0.359*(sAge/1e3)^2 - 1.008*(sAge/1e3) + 19.04
			c76 = c74/c64
		EndIf
...
Function RunActiveDRS() //The DRS function name must be exactly as written here.  Enter the function body code below.
	...
	if(OptionalPartialCrunch!=1)	//if this is a normal crunch data then do all of this stuff, otherwise skip to the 'else' after DownHoleCurveFit()
		...
		If (0&&FindListItem("Hg204", ListOfInputChannels) != -1 && FindListItem("Pb204", ListOfInputChannels) != -1)
		...
		ListOfUPbInputNames = VAU_GenerateUPbInputsList(ListOfInputChannels)
		...
		If ( HoldDHC == 0 )
			VAU_DownHoleCurveFit("Raw_208_232", OptionalWindowNumber = 0)	//the optional window number can be set, if it is it allows the function to stagger the windows so that they don't all overlap completely. if it's missing it defaults to 1
			VAU_DownHoleCurveFit("Raw_207_235", OptionalWindowNumber = 1)
			VAU_DownHoleCurveFit("Raw_206_238", OptionalWindowNumber = 2)		//Note that the reverse order of the ratios here just means that the topmost graph is the most commonly used (i.e. 6/38 ratio)
		EndIf
		// !JAP
		//###########################################################	
...
//	DC208_232 = Raw_208_232
	...
	VAU_DriftCorrectRatios()		//this function has optional range parameters, but they can be left blank and the function will operate on the entire wave
...
Function/T VAU_GenerateUPbInputsList(ListOfInputChannels)
...
Function VAU_DriftCorrectRatios([OptionalMinPoint, OptionalMaxPoint])
...
Function VAU_DownHoleCurveFit(Ratio, [OptionalWindowNumber])
	string Ratio
	...
	strswitch(ShortCurveFitType)	//ShortCurveFitType is the string set using the edit DRS variables window. 
		case "LinExp":
			...
			VAU_FitToAverageWave(ratio, ShortCurveFitType, AutoOrManual)	//send the relevant strings to a separate function that does the curve fit. Note that variables a,c,d are sent, as b is for the linear component above
			...
			SetVariable BVariable Title = "b =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+30-5},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_b, limits={-10,10,(0.0001*average[0])}, format="%-12.4g", userdata = AutoSVarName
			...
			button		LinExpAuto			title="Auto"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+35,TopOfFirstBox+95}, fcolor=(25000,25000,64000),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName		//the userdata is set to AutoSVarName for each control so that this info can be extracted later
			button		LinExpManual		title="Manual"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+130,TopOfFirstBox+95}, fcolor=(0,0,0),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+132},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={-10,10,(0.001*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable CVariable Title = "c =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+164},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_c, limits={-10,10,(0.002*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable DVariable Title = "d =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+196},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_d, limits={-10,10,(0.005*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
		...
		case "Exp":
			...
			button		ExpAuto			title="Auto"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+35,TopOfFirstBox+70}, fcolor=(25000,25000,64000),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName		//the userdata is set to AutoSVarName for each control so that this info can be extracted later
			button		ExpManual			title="Manual"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+130,TopOfFirstBox+70}, fcolor=(0,0,0),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+107},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={-10,10,(0.001*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable BVariable Title = "b =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+139},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_b, limits={-10,10,(0.002*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable CVariable Title = "c =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+171},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_c, limits={-10,10,(0.005*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			if(cmpstr(AutoOrManual, "FullManual")==0)	//all of the above buttons are set up on the assumption that full auto is being used. if it is actually manual, need to change some things:
		...
		case "DblExp":
			...
			button		DblExpAuto			title="Auto"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+35,TopOfFirstBox+35}, fcolor=(25000,25000,64000),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName		//the userdata is set to AutoSVarName for each control so that this info can be extracted later
			button		DblExpManual			title="Manual"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+130,TopOfFirstBox+35}, fcolor=(0,0,0),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+72},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={-10,10,(0.001*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable BVariable Title = "b =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+104},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_b, limits={-10,10,(0.002*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable CVariable Title = "c =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+136},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_c, limits={-10,10,(0.005*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable DVariable Title = "d =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+169},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_d, limits={-10,10,(0.005*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable EVariable Title = "e =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+201},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_e, limits={-10,10,(0.005*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			if(cmpstr(AutoOrManual, "FullManual")==0)	//all of the above buttons are set up on the assumption that full auto is being used. if it is actually manual, need to change some things:
		...
		case "Lin":
			...
			button		LinAuto			title="Auto"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+35,TopOfFirstBox+115}, fcolor=(25000,25000,64000),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName		//the userdata is set to AutoSVarName for each control so that this info can be extracted later
			button		LinManual			title="Manual"	, win=$WindowName ,fsize=15, font="Lucida Grande", pos={LeftOfBoxes+130,TopOfFirstBox+115}, fcolor=(0,0,0),size={80,20},proc=VAU_AutoManButton, userdata = AutoSVarName
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+147},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={-10,10,(0.001*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			SetVariable BVariable Title = "b =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+179},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_b, limits={-10,10,(0.0001*average[0])}, format="%-12.6g", disable=2, userdata = AutoSVarName
			if(cmpstr(AutoOrManual, "FullManual")==0)	//all of the above buttons are set up on the assumption that full auto is being used. if it is actually manual, need to change some things:
		...
		case "RunMed":
			...
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+107},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={1,200,4}, format="%-12.6g", userdata = AutoSVarName
			break
		case "Spline":
			if(cmpstr(AutoOrManual,"Initialise")==0)	//do we need to initialise stuff? i.e. is this the first time the fit has been performed (on this ratio using this fit type)?
			...
			GroupBox 	SmthComponent 		title="Smoothing", win=$WindowName ,pos={LeftOfBoxes,TopOfFirstBox+80},size={250,65}, fsize = 14, frame = 1
			SetVariable AVariable Title = "a =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+90,TopOfFirstBox+107},size={100,20}, bodyWidth=115, frame=0, proc=VAU_VariableFitUpdate, value=Variable_a, limits={.001,200,0.05}, format="%-12.6g", userdata = AutoSVarName
			break
	endswitch
	...
	SetVariable StartMask Title = "start trim (seconds) =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+100,TopOfMaskingBox+18},size={130,20}, bodyWidth=60, frame=0, proc=VAU_VariableFitUpdate, value=Start_MaskForFit, limits={0,120,0.25}, userdata = AutoSVarName
	SetVariable EndMask Title = "end trim (seconds) =",fsize=14, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+100,TopOfMaskingBox+40},size={130,20}, bodyWidth=60, frame=0, proc=VAU_VariableFitUpdate, value=End_MaskForFit, limits={0,120,0.25}, userdata = AutoSVarName
	doupdate
	VAU_MaskStartOrEnd(ratio, WindowName, "StartMask", Start_MaskForFit)	//calling this function here creates the grey mask boxes on the graph if required
	VAU_MaskStartOrEnd(ratio, WindowName, "EndMask", End_MaskForFit)
	...
	Checkbox FitGauss			Title = "Show ideal Gaussian curve",fsize=13, font="Lucida Grande", win=$WindowName, pos={LeftOfBoxes+35,TopOfQualityBox+227},size={130,20}, mode=0, value = 1, proc = VAU_CheckboxFitUpdate, userdata = AutoSVarName
...
Function VAU_FitToAverageWave(ratio, ShortCurveFitType, AutoOrManual)
...
Function VAU_CheckboxFitUpdate(Controlstructure)
	...
	VAU_FitToAverageWave(ratio, ShortCurveFitType, AutoSetting)
...
Function VAU_VariableFitUpdate(Controlstructure)
	...
	if(cmpstr(NameOfButton, "StartMask")==0)	//if the control name is StartMask, then draw a grey box over the relevant masked range
		VAU_MaskStartOrEnd(ratio, WindowName, "StartMask", ControlValue)
	elseif(cmpstr(NameOfButton, "EndMask")==0)	//otherwise if the control name is EndMask, then draw a grey box over the relevant masked range
		VAU_MaskStartOrEnd(ratio, WindowName, "EndMask", ControlValue)
	endif
	VAU_FitToAverageWave(ratio, ShortCurveFitType, AutoSetting)
	OptionalPartialCrunch=1	//set the OptionalPartialCrunch to 1, this means that when the window is deactivated the DRS will be re-crunched
end

Function VAU_MaskStartOrEnd(ratio, WindowName, StartOrEnd, MaskValue)	//this function draws the mask boxes for the relevant areas on the graph. by being a separate function it can be called during a window build
	string ratio
	string WindowName
	string StartOrEnd
	variable MaskValue
...
Function VAU_AutoManButton(buttonstructure)
